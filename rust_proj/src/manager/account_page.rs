...
pub async fn get_account_page(State(app_state): State<AppState>) -> (StatusCode, Html<String>) {
    ...
    let editor_home = AccountPage {
        ...
        selected_languages: Language::vec_from_int_vec(&languages, &response.languages.languages),
...
pub async fn post_language(
    ...
    let res: Vec<i32> = body
        ...
        .collect::<Vec<String>>()
        .iter()
        .map(|item| FromStr::from_str(item.split("=").next().unwrap()).unwrap_or(-1))
        .collect();

    account_model.languages.languages = res;
    set_account_details(&app_state, account_model).await;
    let  info: String = message.to_string();
...
pub async fn post_primary_language(
    ...
    let message = format!("primary_language_updated: {}", id);
    set_account_details(&app_state, account_model).await;
